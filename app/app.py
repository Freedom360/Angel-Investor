        ...
        with st.status("Running Due Diligence Pipeline...", expanded=True) as status:
            try:
                ...
                with col1:
                    ...
                    # Big Metric 
                    delta_text = f"-{eval_data.penalty_total} Penalty" if eval_data.penalty_total > 0 else None
                    st.metric("Confidence Score", f"{eval_data.confidence_score} / 100", delta=delta_text, delta_color="inverse")
